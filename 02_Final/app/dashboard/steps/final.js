...
export default function DashboardPage() {
  ...
  const [dashboardData, setDashboardData] = useState(null); // Holds { marketData, popularStocksData }
      ...
      <section className='bg-gray-100 p-6 md:p-10 flex flex-col gap-12 md:gap-16'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8'>
            ...
            <div className='bg-white p-4 rounded-lg shadow'>
              ...
              <ul>
                ...
                {dashboardData?.marketData?.topPerformers?.map((stock, index) => (
                ...
                )) || <li className="text-xs text-gray-400 italic">Loading data...</li>}
              </ul>
            </div>
            ...
            <div className='bg-white p-4 rounded-lg shadow'>
              ...
              <ul>
                 {/* MODIFIED: Use data from state with optional chaining */}
                {dashboardData?.marketData?.topLosers?.map((stock, index) => (
                  <li key={index} className='flex justify-between items-center py-1 text-sm'>
                    <span className='font-medium text-gray-700'>{stock.name}</span>
            ...
            <div className='bg-white p-4 rounded-lg shadow'>
              ...
              <ul>
                 {/* MODIFIED: Use data from state with optional chaining */}
                {dashboardData?.marketData?.topMovers?.map((stock, index) => (
                  <li key={index} className='flex justify-between items-center py-1 text-sm'>
                    <span className='font-medium text-gray-700'>{stock.name}</span>
        ...
        <div>
          ...
          <div className='flex flex-wrap gap-6 justify-center md:justify-start'>
             {/* MODIFIED: Use data from state with optional chaining */}
            {dashboardData?.popularStocksData?.map((stock) => (
              // Use unique key like stock.name if available and unique
              <Stock key={stock.name || stock.price} stock={stock} />
            )) || <p className="text-gray-500 italic w-full text-center">Loading popular stocks...</p>}
          </div>
        </div>
      </section>
      <Footer />
